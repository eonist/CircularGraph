...
//   }
    ...
    */
   func createGraph(style:CircleGraph.GraphStyle,progress:CGFloat) -> CAShapeLayer{
      let circleCenter = CGPoint.init(x: frame.width/2, y: frame.height/2)
      let radius = frame.height/2
      return with(CAShapeLayer()) { circleLayer in
         self.layer.addSublayer(circleLayer)
         CircleUtil.drawCircle(with: circleLayer, circle:(circleCenter,radius), style:(.clear,style.color,style.strokeWidth), progress: progress)
      }
   }
}
